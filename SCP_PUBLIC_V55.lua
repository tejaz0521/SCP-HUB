...
if _G.scpLibrary then
    ...
    library = loadstring(game:HttpGet("https://raw.githubusercontent.com/memejames/elerium-v2-ui-library//main/Library", true))()
...
local kWin, kFrame = library:AddWindow("🔑 SCP HUB — Key System", {
    ...
    title_bar={Color3.fromRGB(40,40,40),Color3.fromRGB(15,15,15)},
...
kt:AddLabel("⚡  SCP HUB  •  Muscle Legends V5  •  TEJAZ")
kt:AddLabel("🔑  Get your FREE key at  discord.gg/KDx3D8hARN")
local stLbl = kt:AddLabel("📋  Paste key below then press Enter")
...
infoTab:AddLabel("🔑  Key      discord.gg/KDx3D8hARN")
...
farmTab:AddSwitch("⚡  Auto Lift (FireServer rep)", function(v)
    ...
    if v then task.spawn(function() while getgenv().scpAL do
        pcall(function()
            ...
            RS.rEvents.machineInteractRemote:InvokeServer("rep")
        end); task.wait(0.1)
    end end) end
end)
...
farmTab:AddSwitch("🏋️  Auto Bench Press", function(v)
    ...
    if v then task.spawn(function() while getgenv().scpBP do
        pcall(function() RS.rEvents.machineInteractRemote:InvokeServer("rep","bench") end); task.wait(0.05)
    end end) end
end)
...
petsTab:AddSwitch("⚡  Auto Buy Ultimate", function(v)
    ...
    if v then task.spawn(function() while getgenv().scpUlt do
        pcall(function()
            ...
            if f then RS.rEvents.ultimatesRemote:InvokeServer(f) end
        end); task.wait(1)
    end end) end
end)
