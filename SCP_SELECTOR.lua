...
selTab:AddButton("Load Muscle Legends - Free Script", function()
    ...
    task.spawn(function()
        ...
        local ok,err=pcall(function() loadstring(game:HttpGet("https://raw.githubusercontent.com/tejaz0521/SCP-HUB/refs/heads/main/SCP_PUBLIC_V5.lua%20(23).txt",true))() end)
...
selTab:AddButton("Load Blox Fruits Script", function()
    ...
    task.spawn(function()
        pcall(function() winF.Visible=false end)
        local ok,err=pcall(function() loadstring(game:HttpGet(BASE.."SCP_BLOXFRUITS.lua",true))() end)
        if not ok then notify("SCP HUB","Error: "..tostring(err):sub(1,50),5) end
    end)
end)
